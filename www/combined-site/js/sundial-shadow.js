sundialShadowpackage.onWindowResize= function() {
	this.windowHalfX = sundialShadowpackage.forcedWidth/ 2;//window.innerWidth / 2;
	this.windowHalfY = sundialShadowpackage.forcedHeight/ 2;//window.innerHeight / 2;

	this.camera.aspect = sundialShadowpackage.forcedWidth/sundialShadowpackage.forcedHeight; //window.innerWidth / window.innerHeight;
	this.camera.updateProjectionMatrix();

	webglRenderer.setSize(sundialShadowpackage.forcedWidth,sundialShadowpackage.forcedHeight); //window.innerWidth, window.innerHeight);
};

sundialShadowpackage.animate= function() {
	if(!sundialShadowpackage.go) {
...
sundialShadowpackage.reRender= function() {
	var lat= Number(document.getElementById('current-latitude').value);
	var lon= Number(document.getElementById('current-longitude').value);
...
sundialShadowpackage.reRender3= function(az,el) {
	...
	*/
	...
	var cartesian= astropackage.azElToCartensian(lon, lat, alt);
	
	sundialShadowpackage.light.position.x = cartesian.x;
	sundialShadowpackage.light.position.y = cartesian.y;
	sundialShadowpackage.light.position.z = cartesian.z;
...
sundialShadowpackage.setInitialGui= function() {
...
sundialShadowpackage.createWorld= function() {
	...
	this.camera = new THREE.PerspectiveCamera(30, sundialShadowpackage.forcedWidth/sundialShadowpackage.forcedHeight, 1, 100000); //window.innerWidth / window.innerHeight, 1, 100000);
	...
	if(false) {
		...
		loader.load('./img/sundial4.jpg', function(texture) {
			...
			var geometry = new THREE.PlaneGeometry(sundialTwopackage.canvasWidth, sundialTwopackage.canvasHeight);
...
sundialShadowpackage.render= function() {
	...
	sundialShadowpackage.camera.lookAt(this.scene.position);
	sundialShadowpackage.renderer.render(this.scene, this.camera);
};
...
sundialShadowpackage.rotate= function() {
	//camera x=-130.52171101515862, y=1080, z=-991.445451325324
	sundialShadowpackage.camera.position.x= -130.521711; //583.0;
	sundialShadowpackage.camera.position.y= 1080; //1028.0;
	sundialShadowpackage.camera.position.z= -991.44545; //-812.0;

	sundialShadowpackage.star.position.x= 3; //13;//0;
	sundialShadowpackage.star.position.y= 88;//59;
	sundialShadowpackage.star.position.z= -154; //-200;
	sundialShadowpackage.star.rotation.set(0, Math.PI * 1.5, 0);

	sundialShadowpackage.star.castShadow = true;
	sundialShadowpackage.star.receiveShadow = true;
	sundialShadowpackage.star.overdraw = true;
...
sundialShadowpackage.createFromCanvas= function() {
	try {
		var theCanvas = document.getElementById("cnvs-three");
		if (!theCanvas || !theCanvas.getContext) {
			document.getElementById("message-cmp").innerHTML = "Sorry, your browser doesn't support canvas graphics.";
			return;
		}
		try {// try to create a WebGLRenderer
			if (window.WebGLRenderingContext) {
				sundialShadowpackage.renderer = new THREE.WebGLRenderer({
					canvas : theCanvas,
					antialias : true
				});
				console.log("WIDTH: "+ this.SCREEN_WIDTH+", HEIGHT: "+ this.SCREEN_HEIGHT);
				sundialShadowpackage.renderer.setSize(this.SCREEN_WIDTH, this.SCREEN_HEIGHT);
				sundialShadowpackage.renderer.domElement.style.position = "relative";
				sundialShadowpackage.renderer.shadowMapEnabled = true;
				sundialShadowpackage.renderer.shadowMapSoft = true;
			}
		} catch (e) {
		}
		if (!sundialShadowpackage.renderer) {// If the WebGLRenderer couldn't be created, try a CanvasRenderer.
			sundialShadowpackage.renderer = new THREE.CanvasRenderer({
				canvas : theCanvas
			});
			sundialShadowpackage.renderer.setSize(theCanvas.width, theCanvas.height);
			document.getElementById("message-cmp").innerHTML = "WebGL not available; falling back to CanvasRenderer.";
		}
		sundialShadowpackage.renderer.setClearColor(0x220033);
		...
		//        camera.position.z = 4;*
		sundialShadowpackage.createWorld();
		sundialShadowpackage.render();
		document.addEventListener("keydown", this.doKey, false);
	} catch (e) {
		document.getElementById("message-cmp").innerHTML = "Sorry, an error occurred: " + e;
	}
};
...
sundialShadowpackage.createWebGl= function() {
	if (false) {
		try {
			...
			sundialShadowpackage.container = document.createElement('div');
			document.body.appendChild(container);

			sundialShadowpackage.container.appendChild(sundialShadowpackage.renderer.domElement);
			window.addEventListener('resize', this.onWindowResize, false);

		} catch (e) {
			document.getElementById("message-cmp").innerHTML = "Sorry, an error occurred on second renderer: " + e;
		}
	}
	sundialShadowpackage.createWorld();
};
...
sundialShadowpackage.setInitial= function(broadObj) {
	...
	sundialShadowpackage.forcedWidth= 400;
	sundialShadowpackage.forcedHeight= 285;
	
	sundialShadowpackage.SCREEN_WIDTH = sundialShadowpackage.forcedWidth- 100; //window.innerWidth - 100;
	sundialShadowpackage.SCREEN_HEIGHT = sundialShadowpackage.forcedHeight- 100; //window.innerHeight - 100;
	...
	sundialShadowpackage.windowHalfX = sundialShadowpackage.forcedWidth/ 2; //window.innerWidth / 2;
	sundialShadowpackage.windowHalfY = sundialShadowpackage.forcedHeight/ 2; //window.innerHeight / 2;
...
sundialShadowpackage.setValue= function(broadObj) {
	...
	if(broadObj.latitude) {
		...
		sundialShadowpackage.doLocationName(broadObj.latitude, broadObj.longitude);
		
		if(!sundialTwopackage.currentCreated) {
			sundialTwopackage.createDialCurrent(broadObj);
			sundialTwopackage.currentCreated= true;
		}
	}
	...
	if(null != broadObj.date) {
		utilpackage.log("sundialShadowpackage.setValue: date");
	...
	if(broadObj.azimuth) {
		sundialShadowpackage.doAz(broadObj.azimuth,broadObj.elevation);
	}
};
